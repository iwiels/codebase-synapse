...
fn extract_file(tool_name: &str, inp: &Value) -> Option<String> {
    match tool_name {
        ...
        | "multi_replace_file_content" => inp
            .get("file_path")
            .or_else(|| inp.get("TargetFile"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string()),
        _ => None,
    }
...
fn run() -> Result<()> {
    ...
    let pr_threshold = std::env::var("GUARD_PAGERANK_MIN")
        ...
        .unwrap_or(DEFAULT_PR_THRESHOLD);
    let blast_threshold = std::env::var("GUARD_BLAST_MIN")
        .ok()
        .and_then(|v| v.parse::<i64>().ok())
        .unwrap_or(DEFAULT_BLAST_THRESHOLD);
    ...
    match result {
        Ok((pagerank, blast_radius))
            if pagerank >= pr_threshold || blast_radius >= blast_threshold =>
        {
            println!(
                "{}",
                block(format!(
