...
fn insert_project(conn: &Connection) -> i64 {
    conn.execute("INSERT INTO projects (name, root_path) VALUES ('test', '/tmp')", []).unwrap();
...
fn test_leiden_persists_clusters() {
    let (_dir, conn) = setup();
    let pid = insert_project(&conn);
    let nodes: Vec<i64> = (0..6).map(|i| {
        let fp = if i < 3 { format!("src/api/{}.rs", i) } else { format!("src/db/{}.rs", i) };
