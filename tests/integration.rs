...
"#,
    ...
    // 4. Search Code
    let search_code_req = tool_call_json(
        "search_code",
        json!({
            "project": proj_name,
            "query": "greet"
    ...
    assert!(
        !structures.is_empty(),
        "get_file_structure should return symbols"
    );

    // 6. Memory Store & Search
    let mem_store_req = tool_call_json(
        "memory_store",
        json!({
            "project": proj_name,
    ...
    let mem_search_req = tool_call_json(
        "memory_search",
        json!({
            "project": proj_name,
            "query": "additions"
    ...
    assert!(
        ...
        "memory_search should return the note"
    );

    // 7. Get Project Overview
    let overview_req = tool_call_json(
        "project_overview",
        json!({
            "project": proj_name
        }),
    ...
    let task_context_req = tool_call_json(
