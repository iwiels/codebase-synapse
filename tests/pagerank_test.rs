...
fn test_pagerank_inner_hub_beats_leaf() {
    ...
    assert!(
        ranks[&2] > ranks[&1],
        "hub (node2) rank must exceed leaf (node1): node1={} node2={}",
        ranks[&1],
        ranks[&2]
    );
}
...
fn test_pagerank_warm_start_same_result() {
    ...
    for &id in &nodes {
        assert!(
            (cold[&id] - warm[&id]).abs() < 1e-9,
            "warm-start must give same result as cold for node {}",
            id
        );
    }
}
