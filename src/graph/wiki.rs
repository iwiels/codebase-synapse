...
impl Default for WikiConfig {
    fn default() -> Self {
        Self {
            project_name: "this codebase".into(),
...
pub fn render_wiki(
    ...
    for cid in &sorted_ids {
        ...
        for fp in sorted_files.iter().take(config.max_files_per_cluster) {
            let has_v = violations
                .iter()
                .any(|v| &v.from_file == fp || &v.to_file == fp);
            if has_v {
...
fn compute_modularity(assignments: &HashMap<i64, i64>, edges: &[(i64, i64)]) -> f64 {
    ...
    let degrees: HashMap<i64, f64> = assignments
        .keys()
        .map(|&n| {
            (
                n,
                adj.get(&n).map(|a| a.values().sum::<f64>()).unwrap_or(0.0),
            )
        })
        .collect();
...
pub fn generate_wiki(conn: &Connection, project_id: i64, config: &WikiConfig) -> Result<String> {
    let mut stmt =
        conn.prepare("SELECT file_path, cluster_id FROM file_clusters WHERE project_id = ?1")?;
    ...
    let node_ids: Vec<(i64, String)> = {
        let mut stmt2 = conn.prepare("SELECT id, file_path FROM nodes WHERE project_id = ?1")?;
    ...
    let node_to_cluster: HashMap<i64, i64> = node_ids
        .iter()
