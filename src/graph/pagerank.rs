...
impl Default for PageRankConfig {
    fn default() -> Self {
        Self { damping: 0.85, iterations: 30, epsilon: 1e-5 }
    }
}
...
pub fn pagerank_inner(
    ...
    for _ in 0..max_iter {
        ...
        for &node in nodes {
            let rank_sum: f64 = inbound.get(&node)
...
pub fn compute_pagerank(conn: &Connection, project_id: i64, config: &PageRankConfig) -> Result<()> {
    ...
    let ranks = pagerank_inner(&nodes, &edges, config.damping, config.iterations, config.epsilon, &prev);

    let tx = conn.unchecked_transaction()?;
    for (&node_id, &rank) in &ranks {
        let changed = prev.get(&node_id).is_none_or(|&p| (rank - p).abs() >= config.epsilon);
        if changed {
