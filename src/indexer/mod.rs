...
impl Indexer {
    ...
    pub fn index_repository(&self, repo_path: &str) -> Result<()> {
        ...
        let project = db::queries::upsert_project(&conn, repo_name, repo_path).map_err(|e| {
            anyhow::anyhow!("Failed to upsert project '{}': {}", repo_name, e)
        })?;
        ...
        let existing_states = db::queries::get_all_file_states(&conn, project_id)
            .unwrap_or_else(|e| {
                warn!("Failed to fetch file states (proceeding without cache): {}", e);
        ...
        let results: Vec<IndexResult> = files
            ...
            .map(|file_path| {
                ...
                let extraction_res = match &parsed {
                    Some(p) => {
                        match extractors::get_extractor(&p.language) {
                            Some(ext) => ext.extract(&source),
                            None => extractors::ExtractionResult { entities: vec![], relations: vec![] },
                        }
                    }
                    None => extractors::ExtractionResult { entities: vec![], relations: vec![] },
                };

                Ok(Some((file_path_str, content_hash_str, source, extraction_res)))
            })
            .collect();
            ...
            match r? {
                Some((file_path_str, content_hash_str, source_str, extraction_res)) => {
                    ...
                    if !node_ids.is_empty() {
                        ...
                        routes::extract_and_insert_routes(&tx, project_id, &file_path_str, &source_str, file_node_id)?;
                        if manifests::is_manifest_file(&file_path_str) {
                            manifests::extract_and_insert_manifest(&tx, project_id, &file_path_str, &source_str, file_node_id)?;
                        }
                        if infra::is_infra_file(&file_path_str) {
                            infra::extract_and_insert_infra(&tx, project_id, &file_path_str, &source_str, file_node_id)?;
                        }
                    }
                    db::queries::upsert_file_state(&tx, project_id, &file_path_str, &content_hash_str)?;
                    indexed += 1;
                }
                None => {
                    skipped += 1;
        ...
        match crate::similarity::find_similar_pairs(&conn, project_id, 0.70) {
            Ok(pairs) => {
                ...
                for (id1, id2, score) in pairs {
                    ...
                    if let Err(e) = db::queries::insert_edge(&conn, project_id, id1, id2, "similar_to", Some(&metadata)) {
        ...
        // Step 7: Compute Leiden clusters
        {
            ...
            match crate::graph::compute_clusters(&conn, project_id, &leiden_cfg) {
                Ok(r) => info!("Leiden: {} clusters, modularity={:.4}", r.cluster_count, r.modularity),
    ...
    pub fn incremental_update(&self, repo_path: &str, changed_files: &[String]) -> Result<()> {
        ...
        for file_path in changed_files {
            ...
            if !full_path.exists() {
                ...
                updates.push(FileUpdate {
                    ...
                    source: String::new(),
                    extraction: extractors::ExtractionResult { entities: vec![], relations: vec![] },
                    old_nodes: vec![],
                });
                continue;
            }
            ...
            let parsed = match parser::parse_file(&full_path, &source)? {
                Some(p) => Some(p),
                None => {
                    let is_special = manifests::is_manifest_file(file_path) || infra::is_infra_file(file_path);
                    if is_special { None } else { continue; }
                }
            };
            ...
            let old_nodes = {
                let conn = self.conn.lock().map_err(|e| anyhow::anyhow!("DB lock poisoned: {}", e))?;
                db::queries::get_nodes_by_file(&conn, project_id, file_path)
                    .unwrap_or_default()
            };
        ...
        // Phase 3: Write to DB in a single transaction (atomic)
        {
            ...
            for update in &updates {
                ...
                db::queries::delete_nodes_by_file(&tx, project_id, &update.file_path)?;
                let node_ids = builder.add_file_nodes(&update.file_path, update.extraction.clone())?;
                if !node_ids.is_empty() {
                    let file_node_id = node_ids[0];
                    routes::extract_and_insert_routes(&tx, project_id, &update.file_path, &update.source, file_node_id)?;
                    if manifests::is_manifest_file(&update.file_path) {
                        manifests::extract_and_insert_manifest(&tx, project_id, &update.file_path, &update.source, file_node_id)?;
                    }
                    if infra::is_infra_file(&update.file_path) {
                        infra::extract_and_insert_infra(&tx, project_id, &update.file_path, &update.source, file_node_id)?;
                    }
                }
                Self::record_semantic_diff(&tx, project_id, &update.file_path, &update.old_nodes, &update.extraction.entities)?;
                db::queries::upsert_file_state(&tx, project_id, &update.file_path, &update.content_hash)?;
            }
            ...
            info!("Resolving call graph (incremental, {} files)...", changed_count);
            calls::resolve_project_calls(&conn, project_id)?;

            // Phase 4: Conditional expensive recomputation
            // Skip similarity when <5 files changed — O(n²) not worth it for small edits
            if changed_count >= 5 {
                info!("Running structural similarity detection ({} files changed)...", changed_count);
                match crate::similarity::find_similar_pairs(&conn, project_id, 0.70) {
                    Ok(pairs) => {
                        for (id1, id2, score) in pairs {
                            let metadata = serde_json::json!({ "jaccard_score": score }).to_string();
    ...
    fn resolve_test_contracts(
        ...
        let test_files: Vec<(i64, String)> = conn.prepare(
            "SELECT id, file_path FROM nodes WHERE project_id = ?1 AND kind = 'file'"
        ...
        for (file_node_id, file_path) in &test_files {
            let full_path = std::path::Path::new(repo_path).join(
            ...
            let tested_symbols = crate::parser::extractors::extract_tested_symbols(
                &source, &parsed.language
            );
            for symbol in &tested_symbols {
                let targets: Vec<i64> = conn.prepare(
                ...
                for target_id in targets {
                    db::queries::insert_edge(conn, project_id, *file_node_id, target_id, "test_of", None)?;
                }
            }
        }
        info!("Test contracts resolved for {} test files", test_files.len());
        Ok(())
    }
    ...
    fn record_semantic_diff(
        ...
        let old_names: std::collections::HashSet<&str> = old_nodes.iter()
            .filter_map(|n| n.name.as_deref()).collect();
        let new_names: std::collections::HashSet<&str> = new_entities.iter()
            .filter_map(|e| e.name.as_deref()).collect();

        for name in old_names.difference(&new_names) {
            db::queries::insert_semantic_change(
                conn, project_id, file_path, Some(name), "function",
        ...
        for old_node in old_nodes {
            if let Some(old_name) = &old_node.name {
                if let Some(new_e) = new_entities.iter().find(|e| e.name.as_deref() == Some(old_name.as_str())) {
                    ...
                    if delta.abs() > 5 {
                        ...
                        db::queries::insert_semantic_change(
                            conn, project_id, file_path, Some(old_name), &old_node.kind,
                            &format!("'{}' {} in complexity by {} lines", old_name, dir, delta.abs()),
                        )?;
                    }
                }
            }
...
mod tests {
    ...
    fn test_incremental_adds_new_file() {
        ...
        // Verify node exists
        {
            ...
            let nodes: Vec<String> = conn.prepare(
                "SELECT name FROM nodes WHERE project_id = 1 AND kind = 'function'"
            ).unwrap()
        ...
        indexer.incremental_update(repo.to_str().unwrap(), &[abs]).unwrap();
    ...
    fn test_incremental_removes_deleted_file() {
        ...
        {
            let conn = indexer.conn.lock().unwrap();
            let count: i64 = conn.prepare(
                "SELECT COUNT(*) FROM nodes WHERE project_id = 1 AND name = 'gone'"
            ).unwrap()
    ...
    fn test_incremental_skips_unchanged_file() {
        ...
        let conn = indexer.conn.lock().unwrap();
        let count: i64 = conn.prepare(
            "SELECT COUNT(*) FROM nodes WHERE project_id = 1 AND name = 'stable'"
        ).unwrap()
        .query_row([], |row| row.get(0)).unwrap();
        assert_eq!(count, 1);
    }
    ...
    fn test_incremental_records_semantic_diff() {
        ...
        let conn = indexer.conn.lock().unwrap();
        let changes: Vec<String> = conn.prepare(
            "SELECT change_summary FROM semantic_changes WHERE project_id = 1"
        ).unwrap()
        .query_map([], |row| row.get(0)).unwrap()
        .filter_map(|r| r.ok())
        .collect();

        assert!(changes.iter().any(|c| c.contains("old_func") && c.contains("removed")));
        assert!(changes.iter().any(|c| c.contains("new_func") && c.contains("introduced")));
    }
}
